...
void device_open(ftdi_context_t *c) {
    ...
    if (c->status != FT_OK || !c->handle) {
        die(err[DEV_ERR_CANT_OPEN], __func__);
    }
...
u32 device_getver(ftdi_context_t *c, u32 *magic) {
    ...
    *magic = swap_endian(r.resp_immb);
    ...
    if (*magic != DEV_MAGIC)
        die(err[DEV_ERR_BADMAGIC], __func__);
    if (val == 0)
        die(err[DEV_ERR_UNKNOWN], __func__);
...
void device_sendcmd(ftdi_context_t *c, dev_cmd_resp_t *resp,
    ...
    device_checkwrite(c, __func__);
    if (has_resp && !has_dma) {
        if (resp == NULL) die(err[DEV_ERR_NULL_PTR], __func__);
    ...
    if (!has_dma) {
        ...
        if (memcmp(rx_buf, &rx_buf[1], 4) != 0) {
            die("Received wrong CMPlete signal", __func__);
        }
