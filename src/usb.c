...
void parse_args(game_files_t *g, upgrade_files_t *u, int argc, char *argv[]) {
    ...
    printf(" (c) 2014-2017 Retroactive\n");
    printf(" Compiled %s\n\n", compile_date);
        ...
        if (argv[i][0] == '-') {
            ...
            while (1) {
                ...
                if (argv[i][0] == '-') {
                    ...
                    n[t]++;
                    if (n[t] == 16) die("Too many values for argument", __func__);
                }
            }
            t++;
            if (t == 16) die("Too many arguments", __func__);
        }
    }
    ...
    for (i = 0; i < t; i++) {
        switch (f[i]) {
            case 'l':
                if (n[i] < 1) die("Not enough arguments for parameter 'l'", __func__);
                ...
                if (g->types[g->num_files] >= BANK_LAST ||
                    ...
                    die("Specified bank not in valid range", __func__);
                g->num_files++;
                mode |= MODE_LOADIMAGE_BANK;
                break;

            case 'd':
                if (n[i] < 4) die("Not enough arguments for parameter 'd'", __func__);
                strcpy(g->files[g->num_files], a[i][0]);
                ...
                if (g->sizes[g->num_files] < 512) die("Size too small", __func__);
                g->num_files++;
                g->dump = 1;
                mode |= MODE_DUMPIMAGE_BANK;
                break;
            case 's':
                if (n[i] < 1) die("Not enough arguments for parameter 's'", __func__);
                sscanf(a[i][0], "%d", &g->save_types[g->num_files]);
                // check range
                if (g->save_types[g->num_files] >= SAVE_LAST ||
                    g->save_types[g->num_files] < 0)
                    die("Specified savetype not in valid range", __func__);
                ...
                break;
            case 'c':
                if (n[i] < 1) die("Not enough arguments for parameter 'c'", __func__);
                sscanf(a[i][0], "%d", &g->cic_types[g->num_files]);
                // check range
                /*
                if (g->cic_types[g->num_files] >= CIC_LAST ||
                    g->cic_types[g->num_files] < 0)
                    die("Specified cictype not in valid range", __func__); */
                ...
                break;
            case 'f':
                if (n[i] < 1) die("Not enough arguments for parameter 'f'", __func__);
                ...
                break;
            case 'b':
                if (n[i] < 1) die("Not enough arguments for parameter 'b'", __func__);
            ...
            case 'v':
                ...
                break;
            default:
                die("Unhandled or unknown argument", __func__);
                break;
        }
