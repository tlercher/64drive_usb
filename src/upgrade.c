...
void upgrade_load(upgrade_files_t *u, ftdi_context_t *c) {
    ...
    for (i = 0; i < u->num_files; i++) {
        ...
        _printf("Loading %s for %s upgrade", u->files[i],
                u->firm[i] ? "firmware" : u->bootld[i] ? "bootloader" : "unknown");
        ...
        if (rpk == NULL) die(err[DEV_ERR_CANT_MALLOC], __func__);
        fread(rpk, u->sizes[i], 1, fp);
        fclose(fp);

        if (memcmp(rpk, "RETROACTIVE", 11) == 0) {
            die("This upgrade must be used with the 1.xx USB loader", __func__);
        }

        // unwrap (recursively unpack) the package
        if (pkg_cnt_unwrap(rpk, 2, &temp, blobs, &blob_size, blob_magic, &blob_count) != 0) {
            die("Problem unwrapping the RPK", __func__);
        }
        if (blob_count != 2) {
            die("Unexpected number of blobs in RPK", __func__);
        }
        // grab the metadata
        if (memcmp(blob_magic[0], "PM", 2) != 0) {
            die("Metadata expected but not found", __func__);
        }
        metadata = blobs[0];
        if (u->firm[i] && strcmp(metadata->package_type_text, "Firmware") != 0) {
            die("Package doesn't contain any firmware!", __func__);
        }
        if (u->bootld[i] && strcmp(metadata->package_type_text, "Bootloader") != 0) {
            die("Package doesn't contain any bootloader!", __func__);
        }
        if (strncmp(metadata->target_magic, c->magic, 4) != 0) {
            die("Package isn't meant for this device series", __func__);
        }
        if (strncmp(metadata->target_variant, c->variant, 2) != 0) {
            die("Package isn't meant for this device variant", __func__);
        }
        _printf("%s %s, use -v switch to view more info", metadata->package_type_text, metadata->content_version_text);
        if (c->verbose) {
            printf("\n[Target]\nDesigned for %s %s variant %s\n", metadata->target_product_text, metadata->target_magic,
                   metadata->target_variant);
        ...
        if (memcmp(blob_magic[1], "PA", 2) != 0) {
            die("Asset blob expected but not found", __func__);
        }

        // round up to 512bytes
        new_size = (blob_size[1] / 512 + 1) * 512;
        asset_read = malloc(new_size);
        if (asset_read == NULL) die(err[DEV_ERR_CANT_MALLOC], __func__);
        ...
        for (j = 0; j < 8; j++) {
            ...
            if (memcmp(asset_read, blobs[1], new_size)) {
                ...
                die("Memory integrity check failed! SDRAM malfunction", __func__);
            }
        }
        printf(" passed\n");
        ...
        if (rpk != NULL) free(rpk); else die("Can't free null ptr", __func__);
    }
}
...
void upgrade_firm(ftdi_context_t *c) {
    ...
    if ((upgrade_get_report(c) & 0xF) != UPG_STATUS_READY) {
        die("Upgrade module isn't ready, power cycle?", __func__);
    }
    ...
    while (1) {
        ...
        switch (resp) {
            case UPG_STATUS_RESET:
                die("Upgrade module is stuck in reset", __func__);
                break;
            case UPG_STATUS_READY:
                die("Upgrade module never started", __func__);
                break;
            case UPG_STATUS_INVALID:
                die("Unknown status!", __func__);
                break;
            case UPG_STATUS_BADGEN:
                die("Upgrade module encountered a general error", __func__);
                break;
            case UPG_STATUS_BADVARIANT:
                die("Device didn't accept the mismatched variant", __func__);
                break;
            case UPG_STATUS_BADVERIFY:
                die("Upgrade module failed while verifying the package", __func__);
                break;
            case UPG_STATUS_ER_0:
            case UPG_STATUS_ER_25:
            case UPG_STATUS_ER_50:
            case UPG_STATUS_ER_75:
                _printf("Erasing firmware :");
                break;
            case UPG_STATUS_WR_0:
                _printf("Writing firmware [0%%]:");
                break;
            case UPG_STATUS_WR_25:
                _printf("Writing firmware [25%%]:");
                break;
            case UPG_STATUS_WR_50:
                _printf("Writing firmware [50%%]:");
                break;
            case UPG_STATUS_WR_75:
                _printf("Writing firmware [75%%]:");
                break;
            case UPG_STATUS_SUCCESS:
                _printf("Written successfully! ");
                return;
        }
        if (j++ == 5000) die("Took too long, giving up upgrade", __func__);
    }
}
...
void upgrade_bootld(ftdi_context_t *c, u8 *data, u32 size) {
    ...
    while (i < size) {
        ...
        for (j = 0; j < 16; j++) {
            ...
            device_sendcmd_batch(c, DEV_CMD_PI_WR_BL, 0x5555 * 2, 0xAA00 << 8, tx_ptr);
            tx_ptr += 12;
            device_sendcmd_batch(c, DEV_CMD_PI_WR_BL, 0x2AAA * 2, 0x5500 << 8, tx_ptr);
            tx_ptr += 12;
            device_sendcmd_batch(c, DEV_CMD_PI_WR_BL, 0x5555 * 2, 0xA000 << 8, tx_ptr);
            tx_ptr += 12;
            device_sendcmd_batch(c, DEV_CMD_PI_WR_BL_LONG, i, word, tx_ptr);
            tx_ptr += 12;
            i += 2;
        }
